...
구현 상태: 구현. 평소 검색은 갱신 잡이 미리 채운 Redis를 읽어 `fresh: false`로 응답하고, 첫 갱신 전·범위 밖 날짜·Redis 장애로 값이 없는 숙소와 `fresh=true`일 때만 공급사를 직접 호출해요. 상세는 [architecture 4.8](architecture.md).
...
| `checkOut` | `YYYY-MM-DD` | 예 | 체크아웃일. 숙박일에 포함되지 않아요. 9/1\~9/4는 3박 | `checkIn`보다 뒤, 박수 ≤ 30, 설정은 `search.max-nights` |
| `adults` | int | 예 | 성인 수 | 1 이상 |
| `children` | int | 아니요, 기본 0 | 아동 수 | 0 이상 |
| `fresh` | boolean | 아니요, 기본 false | true면 캐시를 건너뛰고 공급사에 직접 물어요. 예약 직전 재확인용 | |
| `includeSoldOut` | boolean | 아니요, 기본 false | true면 예약 가능 객실 수가 0인 객실 타입도 응답에 넣어요 | |

검색 조건은 날짜와 인원뿐이고 대상은 자사가 보유한 숙소 전체, 즉 인메모리 매핑의 active 숙소예요. 검증에 어긋나면 공급사를 부르지 않고 400이에요.
...
| `stays[].hotelId` | long | 내부 숙소 식별자, 매핑에서 |
| `stays[].hotelName` | string | 숙소명, 매핑에서 |
| `stays[].supplier` | `A` / `B` | 출처 공급사 |
| `stays[].roomTypes[]` | array | 객실 타입. `roomTypeId` 오름차순 |
| `roomTypes[].roomTypeId` | long | 내부 객실 타입 식별자, 매핑에서 |
| `roomTypes[].roomTypeName` | string | 객실 타입명, 매핑에서 |
| `roomTypes[].maxOccupancy` | int | 객실 1실 최대 수용 인원, 성인과 아동 합산. 재고·요금 응답 값, 없으면 매핑 값. 둘 다 없는 객실 타입은 응답에서 빼요 |
...
### 2.1 조회 대상과 공급사별 묶음
인메모리 매핑 `MappingRegistry`의 active 숙소 전체가 대상이에요. 먼저 Redis에서 숙소별 객실 타입 × 숙박일 필드를 파이프라인으로 읽고, 값이 없는 숙소만 공급사별로 묶어 `fetchAvailability`를 불러요. `fresh=true`면 전부 불러요. 갱신 잡의 마지막 시도가 실패한 공급사는 `failures`에 그 원인으로 표시해요. 요청당 50개인 공급사 한도는 어댑터가 알고 안에서 잘라 병렬 호출하며, 묶음 일부가 실패하면 성공한 항목과 실패 묶음을 함께 돌려줘요. active 숙소가 없는 공급사는 부르지 않아요. 공급사 간 호출은 병렬이고, 실패는 공급사 단위로 가둔 뒤 병합해요. 상세는 [architecture 4.4](architecture.md).

### 2.2 연박 예약 가능 객실 수 판정
`availableRooms` = 체크인일부터 체크아웃 전날까지의 날짜별 `remainingRooms` 중 최솟값. 하루라도 0이면 0이고 예약 불가예요. 요금은 A는 날짜별 (nightlyRate + taxAmount)의 합, B는 totalPrice 그대로. 응답이 객실 타입 단위라 호실 배정·업그레이드·분할 예약은 다루지 않아요.
...
### 2.4 병합 규칙
1. 공급사 코드를 매핑으로 내부 식별자로 바꿔요. 매핑에 없는 코드, 즉 지난 새벽 이후 추가된 상품은 버리고 로그를 남겨요. 다음 새벽 크론잡이 채우면 그때부터 나와요.
2. 최대 수용 인원은 재고·요금 응답 값 → 매핑 값 순으로 채우고, 둘 다 없으면 그 객실 타입을 빼요. 계약상 필수 필드를 비워 보내지 않기 위해서예요.
3. 인원 필터: `maxOccupancy ≥ adults + children`인 객실 타입만 남겨요.
4. 2.3의 예약 불가 필터.
5. 숙소 단위로 묶고 `hotelId`, `roomTypeId` 오름차순으로 정렬해요. 공급사가 다르면 같은 호텔이어도 다른 숙소로 나와요. 중복 상품 병합은 구현하지 않았어요.
6. 실패는 `failures`로, 조회 대상 공급사가 전부 실패하면 503.

### 2.5 재시도
검색은 사용자가 기다리므로 연결 실패·500·503만 즉시 1회 다시 시도해요. 설정은 `search.retry-attempts`와 `search.retry-delay` 200ms예요. 타임아웃은 이미 3초를 썼고 429는 더 부르면 악화되므로, 그리고 잘못된 요청·인증·깨진 응답은 다시 하지 않아요.
