...
- 기본 흐름
  ...
  3. 값이 없는 숙소만, `fresh=true`면 전부를 공급사별 코드 묶음으로 나눠 재고·요금 API를 병렬 호출해요. 요청당 50개 묶음, 연결·응답 타임아웃, 호출 한도이 적용돼요.
...
- 대안 흐름
  ...
  - A9 Redis에 값이 없거나 Redis 장애: 값이 없는 숙소만, 장애면 전부를 공급사에 직접 호출하는 저하 모드로 가고 `fresh: true`로 표시. 값이 없는 경우는 갱신 잡 첫 갱신 전, 요청 날짜가 오늘\~+30일 캐시 범위 밖, 키 만료예요. 갱신 잡의 마지막 시도가 실패한 공급사는 `failures`에 표시
...
**알림 원칙**: 외부 연결(공급사·DB·Redis)의 실패는 원인이 무엇이든 전부 알려요. 조용히 넘어가는 실패를 두지 않아요. 알림 피로는 "같은 대상·같은 원인은 5분 안에서 하나로 묶기"로 막고, 비율이 임계를 넘으면 긴급으로 올려요.
...
**알림 규칙** (5분 안에서 묶음. 임계치는 보수적으로 시작해 관측으로 조정)
...
2. 429를 받으면 코드가 로그를 남기고 그 공급사의 허용 횟수를 절반으로 줄여요(429마다, 하한 초당 0.5회). 갱신 잡은 지수 백오프(1→2→4→…초, 60초 상한, 5회를 넘기면 이번 갱신 포기)로 물러났다 다시 하고, 검색은 재시도하지 않아요. `Retry-After` 값을 대기에 쓰는 것은 확장 항목이에요.
...
| Redis · 갱신 잡 | 이번 갱신 실패, 다음 주기에 다시 | error | 연속 2회 실패 |
...
[저하 모드]  Redis 가 비었거나(첫 갱신 전·초기화) 연결이 안 되면: 검색이 공급사를 직접 호출(fresh=true 로 표시). 응답 시간 상한 3초는 공급사별 타임아웃이 맡고, 초과분을 pending 으로 표시하는 부분 응답은 설계만. 알림
[콜드 스타트] (설계) 갱신 잡 첫 갱신이 끝나야 웹 팟 readiness 가 올라가요. 롤링 배포라 그동안은 이전 팟이 응답. 지금은 첫 갱신 전 검색이 저하 모드로 응답
```
