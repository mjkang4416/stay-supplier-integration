...
| 초당 호출 횟수 | 공급사당 초당 2회에서 시작(설정값). 한 번에 50개 숙소. 429를 받으면 절반으로 줄이고, 24시간 429가 없으면 20%씩 올리되 설정 상한을 넘지 않음. 예산의 90%를 갱신 잡, 10%를 캐시 창 밖 검색·예약 직전 재확인 같은 실시간 호출에 배분 |
...
| 주기 자동 산출 | 실제 주기 = max(목표 주기, 계층 한 바퀴 호출 수 ÷ 그 계층에 배분된 초당 호출 횟수). 숙소가 늘어 예산이 부족하면 먼 계층부터 주기가 자동으로 늘어난다. B 기준 초당 2회면 약 3,000개 숙소까지 목표 주기를 지킴 |
