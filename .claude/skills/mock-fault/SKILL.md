...
name: mock-fault
description: |
  Mock Supplier 의 공급사별 모드(정상·장애·무응답)를 바꿔 타임아웃·부분 실패·실패 판정 통일을 재현해요.
...
## 전제
- Mock Supplier 가 9090 에 떠 있어요(`run-local` 2번).
- 모드는 재고·요금 조회 API 에만 걸려요. 숙소 목록 API 는 항상 정상이에요.
...
1. 모드 전환: `{a|b}` × `{normal|error|no-response}`
   ...
   ```
2. 검색을 날려 결과를 봐요(`test-search-api` 스킬). 캐시가 차 있으면 검색이 공급사를 부르지 않으므로 먼저 `docker compose exec redis redis-cli FLUSHALL` 로 비워요.
3. 복구
   ```bash
...
- 캐시가 차 있는 상태에서 장애를 걸면 검색은 그대로 캐시로 응답하고, 다음 갱신 바퀴(최대 5분)에서 상태 키에 실패가 기록돼 `failures` 가 붙어요(`query-cache` 3번).

## 주의사항
- 크론잡(`sync` 프로필)은 목록 API 를 쓰므로 이 모드의 영향을 받지 않아요. 목록 실패를 보려면 Mock 을 내리고 크론잡을 돌려요 → `CONNECTION`, 종료 코드 1.
- 모드는 Mock 프로세스 메모리에만 있어 Mock 을 재시작하면 `normal` 로 돌아와요.
