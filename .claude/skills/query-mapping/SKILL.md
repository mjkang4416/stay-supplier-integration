...
name: query-mapping
description: |
  로컬 MySQL 의 매핑 테이블(hotel_mapping, room_type_mapping)을 조회해 크론잡 결과와 내부 식별자 안정성을 확인한다.
...
## 전제
- MySQL 이 Docker 컨테이너 `stay-supplier-mysql` 에서 떠 있고(`docker compose ps`), 매핑 갱신 잡을 한 번 이상 돌렸다.
...
- `max_occupancy` 가 NULL 이면 공급사가 값을 주지 않은 것이다(미상).
- Mock 예제 데이터라면 A-10023(1), A-10044(2), B77120(3)과 객실 타입 세 개가 있어야 한다.

## 주의사항
- DB 초기화가 필요하면 `docker compose down -v` 뒤 `run-local` 스킬 3번(매핑 갱신 잡)을 다시 돌린다. 초기화하면 `id` 가 다시 배정된다.
