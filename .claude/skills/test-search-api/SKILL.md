...
description: |
  통합 검색 API 를 캐시 적중 → 저하 모드 → 부분 실패 → 타임아웃 → 전부 실패 → 매진 포함 → 잘못된 요청 → 실시간 재확인 순서로 curl 로 확인한다.
...
## 전제
- `run-local` 스킬로 MySQL·Redis·Mock·애플리케이션이 떠 있고 매핑이 들어가 있다. 포트를 바꿨으면 아래 8080 을 맞춘다.
...
## 확인 포인트
- 기대값의 근거는 `docs/stay-search-api.md` 1.5 예시와 2장 조회 규칙. 요금은 세금 포함 총액, 예약 가능 객실 수는 기간 내 날짜별 재고의 최솟값이다.
- `failures` 는 200 응답 안에 실패한 공급사와 사유만 담는다. 전부 실패해야 503 이다.
- 4번은 `time_total` 이 3초 부근이어야 한다. 훨씬 짧으면 캐시로 응답한 것이니 2번을 다시 한다.
- 캐시가 차 있는 상태에서 A 를 장애로 두고 5분 기다리면 갱신 잡이 상태 키에 실패를 기록해 캐시 응답에도 `failures` 가 붙는다(`query-cache` 스킬 3번).

## 주의사항
- 3~5번은 캐시가 비어 있어야 검색이 공급사를 직접 부른다. 순서를 지키거나 2번을 먼저 한다.
- 끝나면 6번으로 복구한다. 모드는 Mock 메모리에만 있어 Mock 재시작으로도 돌아온다.
