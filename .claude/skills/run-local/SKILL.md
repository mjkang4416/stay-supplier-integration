...
name: run-local
description: |
  로컬에서 MySQL·Redis(Docker), Mock Supplier, 애플리케이션을 순서대로 띄워요. 처음이거나 DB 를 초기화했으면 매핑 갱신 잡을 먼저 한 번 돌려요.
...
## 전제
- JDK 21 과 Docker 가 있어요. 명령은 모두 저장소 루트에서 실행해요.
- 터미널 세 개를 써요. 1·3번은 한 터미널에서 순서대로, 2번과 4번은 각각 별도 터미널이에요.
...
3. 매핑 갱신 잡 1회 (처음 띄울 때, 또는 DB 를 초기화했을 때)
   ...
   ```
   웹 서버 없이 뜨고 공급사 숙소 목록을 DB 에 넣은 뒤 종료해요.
...
## 주의사항
- `bootRun` 앞의 `:` 를 빼면 Mock 모듈의 bootRun 까지 같이 실행돼요.
...
- Redis 가 없어도 앱은 뜨고 검색은 공급사를 직접 부르는 저하 모드로 동작해요.
