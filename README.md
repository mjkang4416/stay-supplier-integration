...
2·3은 공급사를 호출하는 쪽이 하는 일이다. 평소 검색은 갱신 잡이 5분마다 미리 채운 Redis 캐시를 읽고, 2·3의 직접 호출은 갱신 잡과 캐시에 값이 없는 검색(첫 바퀴 전·창 밖 날짜·Redis 장애·`fresh=true`)에서 일어난다.
...
| 요금/재고 캐시 전략 (선택) | 구현 (Redis 미리 채우기 5분 주기·창 30일, 검색은 Redis 우선, 비었으면 직접 호출, 공급사당 초당 4회 예산과 429 감속, TTL 주기×3, 상태 키). 날짜 거리별 주기·핫 리스트·리더 선출·readiness 연동·`pending` 부분 응답은 설계만 | [architecture.md](docs/architecture.md) |
| 정규화 실패 데이터 격리 (선택) | 부분. 깨진 항목만 버리고 원인과 함께 로그. 격리 저장소는 없음 | [architecture.md](docs/architecture.md) |
| 중복 상품 병합·통화 처리·예약 대행 (선택) | 미구현. 6장 | |
...
- 잃는 것: 갱신 주기만큼 옛 값을 보여줄 수 있고, B의 연박 요금은 연박 할인이 있으면 실제와 다를 수 있다. 둘 다 검색 결과를 참고값으로 두고 예약 직전 재확인으로 흡수한다. Redis가 없거나 비어 있으면 검색이 공급사를 직접 호출하는 저하 모드로 동작한다.
...
#### 대량 숙소 조회
- 선택: 어댑터가 공급사 한도(요청당 50개)를 알고 안에서 잘라 병렬 호출한다. 평소 검색은 미리 채운 Redis만 읽으므로 숙소 수와 무관하게 ms다. Redis가 비었을 때(장애·초기화)만 검색이 직접 호출한다. 호출 예산은 공급사당 초당 4회(설정값)라 1,000개는 3초에 600개, 5초면 전부 캐시에 들어간다. 응답 예산 3초 안에 채운 숙소만 먼저 응답하고 나머지를 `pending`으로 표시하는 부분 응답은 확장 설계로 남긴다(지금은 공급사별 타임아웃 3초가 상한).
...
#### 외부 연결 실패 알림
- 선택: 공급사·DB·Redis 연결의 실패는 원인이 무엇이든 1건 이상이면 알린다(설계). 같은 대상·같은 원인은 5분 창에서 하나로 묶고, 비율이 임계를 넘으면 긴급으로 올린다. 지표를 코드에 넣지 않으므로 알림의 근거는 실패마다 남기는 로그(공급사·원인·원본 코드)다.
- 근거: 조용히 넘어가는 외부 실패가 가장 늦게 발견되는 장애다. 부분 실패를 허용하는 설계일수록 실패가 응답에서 가려지므로 알림으로 보완한다.
- 잃는 것: 실패가 잦은 공급사는 알림이 많아진다. 묶음 창과 임계로 조정한다.
- 자세히: [architecture.md 4.7](docs/architecture.md)
...
- 자세히: [architecture.md 2장](docs/architecture.md)
